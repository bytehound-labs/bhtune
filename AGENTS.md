...
service.rs`, `#[cfg(target_os = "windows")]` glue over the `windows-service` crate for
   ...
   Phase 7.5 (pre-v1 UX and terminology hardening) is under way, nine of eleven sub-phases
...
servers`/`browse`/`read`) backing the still-to-come GUI OPC browser, each OPC DA call
   bounded by a 30-second timeout. Remaining: `ui-opc-browser` (the GUI server dropdown/
   tag-tree browser/connection test) and `phase75-docs` (the documentation wrap-up). See the
   Status section above for the full design and verification detail behind each.
