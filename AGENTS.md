...
see "`docs-site-scaffold`: the Docusaurus documentation site" below. See "Phases and todos"
...
that publishes `docs/` as a browsable, searchable site — see the `website/` row in "Crate map
and phase status" above for what's done versus still pending (`docs-site-deploy`,
...
9. **Documentation and release** — two prerequisites are already done, front-loaded ahead of
   ...
   "Crate map and phase status" above for the full design. Remaining: publishing that site
   somewhere (`docs-site-deploy`), `cargo doc` published alongside it (`docs-api-rustdoc`),
   release-time version snapshots (`docs-versioning`), a published roadmap doc
