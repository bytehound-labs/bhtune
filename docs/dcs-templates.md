...
| `revert_mode`       | boolean | —                                                                                               | Whether the loop's controller mode is switched back to its original mode (e.g. Auto/Cascade) after a completed test. A live OPC DA run that starts in Auto holds the loop in Manual for one-third of the measured oscillation period after restoring the MV, then releases it to the template-defined Auto value. Has no effect if the loop was already in Manual when the test started. |
...
The same rule is used when an engineer selects a tag in the web UI. For example, browsing
`Area01.FIC101.OUT` with a template whose `process_variable_suffix` is `PV` produces
`Area01.FIC101.PV`. BHTune first reads `Area01.FIC101.OUT` to check its OPC quality, then
replaces the final component; it does not blindly append `.PV`. The preceding path and its
final separator are preserved.
...
<figure>
  <a href="https://bytehound-labs.github.io/bhtune/generated/web-ui/full-opc-tag-browser.png?v=89107e24dd16">
    <img src="https://bytehound-labs.github.io/bhtune/generated/web-ui/full-opc-tag-browser.png?v=89107e24dd16" alt="BHTune OPC tag browser with Area01.FIC101.OUT selected and its quality read visible" />
  </a>
