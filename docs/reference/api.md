...
| [`bhtune_driver`](pathname:///api/bhtune_driver/index.html) | library          | The `Driver` trait (tag read/write/browse) and its implementations: OPC DA (via `opcda-bridge`), an in-process FOPDT simulator, and a golden-trace replay driver for testing. |
