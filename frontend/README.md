...
The header's colored status dot and server version label are vertically centered together. The dot
polls `GET /api/health` every five seconds. Green means the BHTune HTTP service is reachable; it
does not verify OPC DA or another process-driver connection. Hover the dot for the full status
detail.
