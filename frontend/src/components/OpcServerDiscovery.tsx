 ...
 * the bridge gateway, and the New Run form must not make one just because the page loaded
  ...
  return (
    <div className="mt-1">
      ...
      {requested && servers.isError && (
        <p className="mt-1 text-xs text-red-400">{servers.error.message}</p>
      )}
        ...
        <>
          {servers.data.servers.length === 0 ? (
            <p className="mt-1 text-xs text-slate-500">
              No OPC DA servers were found registered on this bridge host.
            </p>
          ) : (
